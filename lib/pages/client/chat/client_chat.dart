...
class _ClientChatPageState extends State<ClientChatPage> {
  ...
  /// ✅ Logged-in client (REAL)
  ChatUser authUser = const ChatUser(id: "", role: "CLIENT", name: "Client");
  ...
  Future<void> _initAuthUser() async {
    ...
    setState(() {
      authUser = ChatUser(
        ...
        name: (user?["name"] ?? "Client").toString(),
      );
    });
  }
  ...
  Future<void> _loadInbox({bool silent = false}) async {
    try {
      ...
      /// ✅ client inbox only
      final list = await repo.inbox(chatType: "CLIENT");
      ...
      if (activeProjectId == null && list.isNotEmpty) {
        ...
        await _fetchOwnerInfo(first.projectId);
        await _loadMessages(first.projectId, markSeen: true);

        if (mounted) setState(() => showProjectListMobile = false);
      }
    } catch (e) {
      debugPrint("❌ CLIENT INBOX ERROR: $e");
      if (!silent && mounted) setState(() => loadingInbox = false);
    }
  }

  /// ✅ Fetch owner phone + ownerId from backend members
  Future<void> _fetchOwnerInfo(String projectId) async {
    try {
      final members = await ProjectService().getProjectMembers(projectId);
  ...
  Future<void> _loadMessages(
    ...
    try {
      ...
      final msgs = await repo.messages(projectId: projectId, chatType: "CLIENT");
      ...
      if (markSeen) {
        await repo.seen(projectId: projectId, chatType: "CLIENT");
        await _loadInbox(silent: true);
      }
    } catch (e) {
      debugPrint("❌ CLIENT MSG ERROR: $e");
  ...
  Future<void> _send() async {
    ...
    /// ✅ Receiver must be REAL ownerId
    final receiver = ChatUser(
      id: ownerId.isNotEmpty ? ownerId : "unknown",
      role: "OWNER",
      name: "Owner",
    );
    ...
    try {
      ...
      await _loadMessages(pid, silent: true);
      await _loadInbox(silent: true);
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
    } catch (e) {
                ...
                SizedBox(
                  ...
                  child: _ProjectListInbox(
                    ...
                    onSelect: (item) async {
                      ...
                      await _fetchOwnerInfo(item.projectId);
