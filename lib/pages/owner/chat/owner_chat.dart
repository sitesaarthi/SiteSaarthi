...
class _OwnerChatPageState extends State<OwnerChatPage> {
  ...
  /// ✅ NEW: active chat type TEAM/CLIENT
  String activeChatType = "TEAM";
  ...
  /// ✅ load actual auth user
  ChatUser authUser = const ChatUser(id: "", role: "OWNER", name: "Owner");
  ...
  Future<void> _initAuthUser() async {
    ...
    setState(() {
      authUser = ChatUser(
        id: (user?["_id"] ?? user?["id"] ?? user?["userId"] ?? "").toString(),
        role: (user?["role"] ?? "OWNER").toString(),
        name: (user?["name"] ?? "Owner").toString(),
      );
    });
  }
  ...
  Future<void> _loadInbox({bool silent = false}) async {
    try {
      ...
      /// ✅ NEW: load TEAM + CLIENT mixed
      final list = await repo.inboxAll();
      ...
      debugPrint("❌ OWNER INBOX ERROR: $e");
  ...
  Future<void> _loadMessages(
    ...
    try {
      ...
      final msgs = await repo.messages(projectId: projectId, chatType: chatType);
      ...
      if (markSeen) {
        await repo.seen(projectId: projectId, chatType: chatType);
        await _loadInbox(silent: true);
      }
    } catch (e) {
      debugPrint("❌ OWNER MSG ERROR: $e");
  ...
  Future<void> _send() async {
    ...
    /// ✅ receiver depends on chat type
    final receiver = activeChatType == "CLIENT"
        ? const ChatUser(id: "client", role: "CLIENT", name: "Client")
        : const ChatUser(id: "team", role: "TEAM", name: "Team");
    ...
    try {
      ...
      await _loadMessages(pid, chatType: activeChatType, silent: true);
      await _loadInbox(silent: true);

      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
    } catch (e) {
      debugPrint("❌ OWNER SEND ERROR: $e");
    }
  }

  Future<void> _callOwner() async {
    final phone = widget.ownerPhoneNumber.trim();
              ...
              if (isWide || !showProjectListMobile)
                Expanded(
                  child: Column(
                    children: [
                      _ChatHeader(
                        title: "$activeProjectName (${activeChatType == "CLIENT" ? "Client" : "Team"})",
                        isWide: isWide,
                          ...
                          child: loadingMessages
                              ...
                              : ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    /// ✅ FIX: compare sender role/id properly
                                    final isMe = msg.sender.id == authUser.id ||
                                        msg.sender.role.toUpperCase() == "OWNER";
                                        ...
                                        Row(
                                          children: [
                                            ...
                                            Container(
                                              ...
                                              decoration: BoxDecoration(
                                                color: p.chatType == "CLIENT"
                                                    ? Colors.blue.shade100
                                                    : Colors.green.shade100,
                                                borderRadius: BorderRadius.circular(999),
                                              ),
                                              child: Text(
                                                p.chatType == "CLIENT" ? "CLIENT" : "TEAM",
                                                style: TextStyle(
                                                  fontSize: 10,
                                                  fontWeight: FontWeight.w900,
                                                  color: p.chatType == "CLIENT"
